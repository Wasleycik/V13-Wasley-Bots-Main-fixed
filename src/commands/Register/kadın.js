...
if(message.channel.id !== config.channels.welcomechannel) return message.reply({content: `Kayıt Komutlarını Sadece Kayıt Kanalında Kullanabilirsin.`}).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        ...
        if (!names) {
            message.reply({ embeds: [embed.setDescription(`${member} adlı üye **Kadın** olarak kayıt Oldu!`)] });
        } else {
        ...
        \`Verilen Roller :\` <@&${config.registration.onewoman}>    
