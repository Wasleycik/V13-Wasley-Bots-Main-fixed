...
const config = require("../../../config.json")
 ...
 if(kullanici.id === message.author.id) return message.reply("Kendi kendini Atamazsın.", message.author, message.channel)
     if (message.guild.members.cache.has(kullanici.id) && message.member.roles.highest.position <= message.guild.members.cache.get(kullanici.id).roles.highest.position) return message.reply("Kendi rolünden yüksek kişilere işlem uygulayamazsın!", message.author, message.channel)
        if(!kullanici){
