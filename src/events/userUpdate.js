...
module.exports = async function(oldUser, newUser) {
    ...
    let ikinciTag = config.registration.TagSymbol;
    ...
    if (newUser.username !== oldUser.username) {
        if (oldUser.username.includes(config.registration.GuilDTag) && !newUser.username.includes(config.registration.GuilDTag)) {
            ...
            if (member.manageable) member.setNickname(member.displayName.replace("✦", "✧"));
            ...
            if (member.manageable) member.setNickname(member.displayName.replace("✧", "✦"));
            client.channels.cache.get(config.channels.chat).send(`${config.emojis.tada} Tebrikler, ${newUser} \`${config.registration.GuilDTag}\` tagımızı alarak ailemize katıldı! Hoşgeldin`)
